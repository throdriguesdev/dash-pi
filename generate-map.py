...
def update_graph(selected_option):
    ...
    if selected_option in ['public', 'both']:
        for region, states in region_map.items():
            fig.add_trace(go.Bar(name=f'Escolas Públicas - {region}', x=states, y=[df1[df1['NO_UF'] == state]['Escolas_Publicas_Com_Biblioteca'].iloc[0] for state in states]))

    if selected_option in ['private', 'both']:
        for region, states in region_map.items():
            fig.add_trace(go.Bar(name=f'Escolas Particulares - {region}', x=states, y=[df2[df2['NO_UF'] == state]['Escolas_Particulares_Com_Biblioteca'].iloc[0] for state in states]))
